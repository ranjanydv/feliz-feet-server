...
async function createOrder(req, res, next) {
  try {
    ...
    await CartProducts.destroy({ where: { user_id: req.authData.user_id } });
...
async function updateOrder(req, res, next) {
  try {
    ...
    const orderExits = await Order.findOne({
      where: {
        id: params.id,
        [Op.and]: [{ state: { [Op.ne]: 2 } }, { state: { [Op.ne]: 3 } }],
      },
    });
    if (!orderExits)
      return res
        .status(400)
        .json({ status: 400, message: "Cannot update order" });
    const orderData = {
      state: body.state,
    };
...
async function cancelOrder(req, res, next) {
  try {
    ...
    const orderExits = await Order.findOne({
      where: {
        id: params.id,
        [Op.and]: [{ state: { [Op.ne]: 2 } }],
      },
    });
    if (!orderExits)
      return res
        .status(400)
        .json({ status: 400, message: "Cannot cancel order" });
    const orderData = {
      state: 3,
    };
    await Order.update(orderData, { where: { id: params.id } });
